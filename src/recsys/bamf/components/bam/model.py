...
class BayesianAttentionModules(nn.Module):
    ...
    def forward(
        ...
        method: str,                        # `estimate` or `perdict`
    ):
        sampler = getattr(self.sampler, method)
        # Q: (B,D) -> (B,1,D) -> (B,H,D)
        Q_exp = Q.unsqueeze(1).expand_as(K)
        # attention scores: (B,H)
        scores, kl_entry = sampler(Q_exp, K)
        # masking: (B,H) -> (B,H)
    ...
    def estimate(
        ...
        """
        return self.forward(Q, K, V, mask, "estimate")
    ...
    def predict(
        ...
        Returns:
            ...
            kl_mean (torch.Tensor):
                ...
                masked for padding indices or correct indices.
        """
        return self.forward(Q, K, V, mask, "predict")
